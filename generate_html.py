import csv from collections import defaultdict, Counter from datetime import datetime import subprocess

def parse_env(hostgroup): if '_prd' in hostgroup: return 'prod' elif '_dev' in hostgroup: return 'nonprod' else: return 'Other'

def parse_location(hostgroup): if hostgroup.startswith('uk-'): return 'UK' elif hostgroup.startswith('za-'): return 'SA' else: return 'Other'

def load_data(csv_file): with open(csv_file) as f: reader = csv.DictReader(f) return list(reader)

def group_by_location_env(hosts): env_location = defaultdict(lambda: defaultdict(int)) os_count = Counter() hostgroup_count = defaultdict(int) grouped_hostgroups = defaultdict(lambda: defaultdict(lambda: defaultdict(int))) uptime_30_90 = defaultdict(lambda: defaultdict(list)) uptime_90_plus = defaultdict(lambda: defaultdict(list))
...
def save_uptime_hosts(grouped_data, filename): with open(filename, 'w') as f: for loc in grouped_data: f.write(f"\n### {loc} ###\n") for env in grouped_data[loc]: f.write(f"\n--- {env.upper()} ---\n") for host in sorted(grouped_data[loc][env], key=lambda x: int(x.get("Uptime", 0)), reverse=True): f.write(f"{host['hostname']} | {host['ip']} | {host['OS Version']} | {host['Uptime']} days\n")

def create_html_report(stats, output_file="report.html"): now = datetime.now().strftime("%Y-%m-%d %H:%M:%S") html = f"""<!DOCTYPE html>
...
def send_inline_email(subject, to_address, html_body): email_msg = f"""Subject: {subject} MIME-Version: 1.0 Content-Type: text/html

{html_body}""" process = subprocess.Popen(["/usr/sbin/sendmail", "-t", to_address], stdin=subprocess.PIPE) process.communicate(email_msg.encode())

if name == "main": csv_file = "hosts.csv" hosts = load_data(csv_file) stats = group_by_location_env(hosts)
...
html_report = create_html_report(stats, "report.html")
send_inline_email("Satellite Host Report", "your@email.com", html_report)
print("✅ HTML report sent and saved.")
